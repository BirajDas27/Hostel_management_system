...
if (isset($_POST['submit'])) {
    ...
    $rc = $stmt->bind_param('iiiisisissssisississsisssi', $roomno, $seater, $feespm, $foodstatus, $stayfrom, $duration, $course, $regno, $fname, $mname, $lname, $gender, $contactno, $email, $emcntno, $gurname, $gurrelation, $gurcntno, $caddress, $ccity, $cstate, $cpincode, $paddress, $pcity, $pstate, $ppincode);
    $stmt->execute();
    $stmt->close();


    $query1 = "insert into  userregistration(firstName,middleName,lastName,gender,contactNo,email,password,course) values(?,?,?,?,?,?,?,?)";
    $stmt1 = $mysqli->prepare($query1);
    $stmt1->bind_param('ssssisss', $fname, $mname, $lname, $gender, $contactNo, $email, $contactNo, $course);
    $stmt1->execute();
    echo "<script>alert('Student Succssfully register and the password is the contact number');</script>";
